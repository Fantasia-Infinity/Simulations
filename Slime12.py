...
class Unit:
    def __init__(self, c, state, c2, c3, c4, life):
        self.c = c  # 信息素浓度
        self.c2 = c2
        self.c3 = c3  # b反捕食者信息素
        self.c4 = c4  # 尸体腐烂的味道
        self.s = state  # 状态：0-空位 1-a生长细胞 2-物体 3-捕食细胞 4-反捕食者 5-尸体 6-分解者
        self.l = life  # 生命值
    ...
    def randinit(self):
        for i in range(self.n):
            for j in range(self.n):
                self.ground[i][j] = Unit(random.random(), 0, 0, 0, 0, 0)

    def addslime(self, x, y):
        self.ground[x][y].s = 1
        self.ground[x][y].l = 100  # 满100
...
#        if sc<10000:
    ...
    def addc4(self, x, y, n):
        sc4 = self.ground[x][y].c4
        self.ground[x][y].c4 = sc4+n
    ...
    def release4(self, x, y):
        k = 10
        dc = self.ground[x][y].l*k
        for f in self.fl:
            i = f(self, x, y)[0]-x
            j = f(self, x, y)[1]-y
            self.addc4(x+i, y+j, dc)
            ...
            if self.ground[x][y].l >= 100:
                for i in [-1, 0, 1]:
                    for j in [-1, 0, 1]:
                        if not (i == j and i == 0):  # 如果生命值满且于内部就给周围加资源
                            if self.ground[x+i][y+j].s == 1:
                                # 通过控制增加的资源数可以调控聚合体稳定后的大小
                                self.ground[x+i][y+j].l += 5
#                self.die(x,y)#然后死亡
        self.move(x, y, nx, ny)
...
#            self.ground[nx][ny].l=100
    ...
    def upperpredator(self, x, y):
        ...
        for f in self.fl:
            i = f(self, x, y)[0]-x
            j = f(self, x, y)[1]-y
            if self.ground[x+i][y+j].s == 1:  # or self.ground[x+i][y+j].s==6:
...
#            self.ground[x][y].l=10
    ...
    def upperglass(self, x, y):
        if self.ground[x][y].l <= 0:
            ...
            self.ground[x][y].l -= 1.5

    def uppershiti(self, x, y):
        self.release4(x, y)
        if self.ground[x][y].l <= 0:
            self.ground[x][y].l = 0
            self.ground[x][y].s = 0
        else:  # 护甲生命值会减少 到零死
            self.ground[x][y].l -= 0.1

    def upperdecomposer(self, x, y):
        nx = x
        ny = y
        mc = self.ground[x][y].c
        mc3 = self.ground[x][y].c3
        mc4 = self.ground[x][y].c4
        k = 0.0
        k3 = 0.0
        k4 = 100
        mytc = k*mc+k3*mc3+k4*mc4
        count = 0
        for f in self.fl:
            i = f(self, x, y)[0]-x
            j = f(self, x, y)[1]-y
            if self.ground[x+i][y+j].s == 5:  # or self.ground[x+i][y+j].s==4:
                self.ground[x+i][y+j].l -= 50
                self.ground[x][y].l += 6
            elif self.ground[x+i][y+j].s == 0:
                if self.ground[x][y].l >= 100:
                    self.ground[x+i][y+j].s = 6
                    self.ground[x+i][y+j].l = 30
                else:
                    nc = self.ground[x+i][y+j].c
                    nc3 = self.ground[x+i][y+j].c3
                    nc4 = self.ground[x+i][y+j].c4
                    netc = k*nc+k3*nc3+k4*nc4
                    if mytc >= netc:
                        pass
                    else:
                        mc = nc
...
#            self.ground[x][y].l=10
    ...
    def kuosan4(self, x, y):
        rate = 0.9
        deltaout = self.ground[x][y].c4*rate
        deltain = 0
        for f in self.fl:
            tx, ty = f(self, x, y)
            try:
                d = self.ground[tx][ty].c4*rate/self.abalcount(tx, ty)
            except:
                d = 0.0
            deltain += d
        self.ground[x][y].c4 += deltain
        self.ground[x][y].c4 -= deltaout
    ...
    def decomposeC4(self, x, y):
        c4 = self.ground[x][y].c4
        k = 0.1
        self.ground[x][y].c4 -= k*c4
    ...
    def uppergroundunit(self, x, y):
        ...
        elif self.ground[x][y].s == 5:
            self.uppershiti(x, y)
        elif self.ground[x][y].s == 6:
            self.upperdecomposer(x, y)
        self.decomposeC2(x, y)
        self.decoposeC(x, y)
        self.decomposeC3(x, y)
        self.decomposeC4(x, y)
...
for i in range(30):
    g.adddecomposer(random.randint(0, g.n-1), random.randint(0, g.n-1))
# g.addslime(25,25)
BLUE = (0, 0, 255)
BLACK = (0, 0, 0)
YELLOW = (0, 255, 0)
...
def drawone(i, j, c):
    ...
    elif c.ground[i][j].s == 5:
        pygame.draw.rect(dissurf, GREY, (i*ge, j*ge, ge-2, ge-2))
    elif c.ground[i][j].s == 6:
        pygame.draw.rect(dissurf, R, (i*ge, j*ge, ge-2, ge-2))
