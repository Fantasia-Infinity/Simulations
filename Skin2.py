    ...
    def setbeing(self):
        for i in range(self.n):
            for j in range(self.n):
                self.a[i][j] = Cell(i, j, random.randint(
                    0, 10), random.randint(0, 10), 0.0)
...
#            deltb=255
    ...
    def update(self):
        ...
        for i in range(self.n):
            for j in range(self.n):
                newskin.a[i][j] = Cell(i, j, self.upper(self.a[i][j])[0], self.upper(
                    self.a[i][j])[1], self.upper(self.a[i][j])[2])
        self.a = newskin.a
