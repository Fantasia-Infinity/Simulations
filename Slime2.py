                ...
                if not (i==j and i==0):
                    if self.inrange(slime.x+i,slime.y+j):
                        if self.ground[slime.x+i][slime.y+j].c<=mc:
                            pass
                        elif self.ground[slime.x+i][slime.y+j].s!=0:
                            pass
                        else:
...
#                nx=f(self,nx,ny)[0]
#                ny=f(self,nx,ny)[1] 
        if mc-self.ground[slime.x][slime.y].c>0.1:   
...
while True:
    ...
    for s in g.l:
        drawslime(s)
    for u in sl:
        pygame.draw.rect(dissurf,BLUE,(u[0]*ge,u[1]*ge,ge-2,ge-2))
